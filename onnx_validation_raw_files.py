...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            #raw file
            scale_coords(image_tensor[si].shape[1:], predn[:, :4], shape, shapes[si][1])  # native-space pred
            ...
            if nl:
                ...
                #raw file
                scale_coords(image_tensor[si].shape[1:], tbox, shape, shapes[si][1])  # native-space labels
        ...
        if plots and batch_i < 3:
            ...
            #raw files
            Thread(target=plot_images, args=(image_tensor, targets, paths, f, names), daemon=True).start()
            
            f = save_dir / f'val_batch{batch_i}_pred.jpg'  # predictions
            Thread(target=plot_images, args=(im, output_to_target(out), paths, f, names), daemon=True).start()
            #raw files
            Thread(target=plot_images, args=(image_tensor, output_to_target(out), paths, f, names), daemon=True).start()
